...
img_width_height = 356#356
network_output_width = 44#44
...
def make_test_batch(batch_size = 1):
    video_path = './video_with_abstacle/for_test/youtube/60frame_video'
    anno_path = './video_with_abstacle/for_test/youtube/60frame_annotation'
...
def make_inspection_batch(batch_size = 1):
    video_path = './video_with_abstacle/60frame_video'
    anno_path = './video_with_abstacle/60frame_annotation'
    ...
    video_path = [video_path +'/'+ video_list[i] for i in index]
    anno_path = [anno_path +'/'+ anno_list[j] for j in index]
    #print("test video : ",shuffled_video_path)
    return np.asarray(video_path), np.asarray(anno_path)
                ...
                for plane_idx, (j_idx1, j_idx2) in enumerate(joint_pairs):
                    if(('%d' %j_idx1 in key) and ('%d' %j_idx2 in key)):
                        ...
                        if not center_from or not center_to:
                            continue
                        _put_paf_on_plane(vectormap=vectormap, countmap=countmap, plane_idx=plane_idx, center_from=center_from, center_to=center_to,                                           threshold=1, height=height, width=width, stride = 1)
    ...
    def train(self):
        ...
        for epoch in range(15):#15
            ...
            for i in range(total_batch):#total_batch
                ...
                heatmap = self.make_heatmap(batch_annotation, width=network_output_width, height=network_output_width, num_of_maps=17)
                vectormap, countmap = self.make_paf_field(batch_annotation, width = network_output_width, height = network_output_width, num_of_maps = 17)
